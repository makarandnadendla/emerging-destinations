...
The prototype in `prototype/` is a 6-slide horizontal deck rendering **mocked** data. It exists to validate the visual story before any real extraction. The real question is empirical: **does origin-country development (HDI) predict how far off the beaten path travelers go inside Georgia (and, stretch, Vietnam)?**
...
**Hard constraint:** 2 weeks aggressive, no Flickr key obtained yet. Scope is **Georgia-only**, Vietnam strictly sequential (only after Georgia ships).
...
| Decision | Choice |
|---|---|
| Destination | **Georgia first; Vietnam only after Georgia fully ships** |
...
|---|---|---|
| `extract_flickr_photos.py` | Flickr `photos.search` (quadtree over Georgia bbox, 2012–2019) | `photos_raw/dt=YYYYMMDD/*.parquet` — one row per photo |
| `extract_flickr_users.py` | Flickr `people.getInfo` for unique user_ids from above | `users_raw/dt=YYYYMMDD/*.parquet` |
| `extract_osm_pois.py` | Geofabrik Georgia PBF + `osmium tags-filter` | `pois_raw.parquet` — one row per POI |
...
| (fallback) `extract_inaturalist.py` | iNat observations API, Georgia bbox | `inat_raw.parquet` |

**Flickr quadtree extraction** (`extract_flickr_photos.py`): recursive bbox subdivision when a query returns ≥4000 results. Pages 500 results at a time (Flickr cap is 250/page for geo queries; the 4000-per-query cap is what triggers subdivision). Persist a `frontier` table in DuckDB so the extraction can resume cleanly. Estimated: 1–3 days wall-clock for Georgia 2012–2019. Rate-limited at ~1 req/sec to stay polite (no documented rate limit but courtesy).
...
2. **Modal-photo-country** (audit): for each user, find the modal country across **all** their geotagged photos (not just Georgia). Used only to flag disagreements and as a sensitivity check — does not replace stated location.

**OSM POI extraction** uses `osmium tags-filter` on the Georgia PBF. POI tag set (subject to one tweak after first inspection):
...
| 03 | `cells` | One row per H3 r6 cell that intersects Georgia. `h3_r6`, `centroid_lat`, `centroid_lon`, `area_km2_in_country` |
...
| 06 | `user_destination` | Filter to tourists: `users.modal_country_iso ≠ 'GE'`, ≥5 photos & ≥2 cells in Georgia. Origin country + year-of-first-photo carried forward |
...
- **Model 2 (supplemental, cell-level multilevel logistic):**
  - One row per (user × cell-that-could-be-visited-in-Georgia). Outcome: visited (1/0).
...
- Endpoint: `flickr.photos.search` with `bbox`, `min_taken_date`, `max_taken_date`, `extras=geo,date_taken,owner_name,tags`, `per_page=500`.
...
- Endpoint: `GET /v1/observations?place_id=<georgia>&per_page=200&order_by=observed_on`.
...
- Download `georgia-latest.osm.pbf` from https://download.geofabrik.de/asia/georgia.html
...
| 2 | OSM POI extraction (Georgia, fast). World Bank + UNDP + WHO indicator pulls. | SQL stubs 01–05. | Outline methodology PDF. |
| 3 | Flickr quadtree extraction kicks off (background). | Cell remoteness model finalized; verify against intuition. | — |
...
**Vietnam stretch:** only if all of the above is complete by day 13. Otherwise documented as future work in PDF.
...
| Georgia sample too small (<300 analysis users) | Med | Med | Loosen threshold to ≥3 photos / ≥2 cells. Document in robustness. |
...
| OSM POI density poorly proxies remoteness | Low | High | Pre-validate against known anchor points (Tbilisi should be lowest remoteness; Tusheti highest). Sanity check first. |
...
1. `python pipeline/run.py --extract` produces parquet artifacts in cloud storage (or `data/cache/` for local dev) with row counts within expected order of magnitude (photos: 10k–500k; users: 1k–30k; POIs: ~50k for Georgia).
...
7. **Sanity check** on the analysis: known central locations (Tbilisi, Kutaisi, Batumi) appear as low-remoteness cells; known peripheral regions (Tusheti, Svaneti highlands) appear as high-remoteness cells. If this fails, the OSM POI definition needs tuning before any inferential claim is made.
...
- **Hour-of-day negative control timezone:** convert photo timestamps to local Georgia time, or origin-country time? Probably destination-local (asking what hour they were in Georgia, not when they would have been awake at home).
...
- **Vietnam-stretch H3 resolution:** Vietnam might want r5 (~250 km²) given the much larger area. Same code, different resolution. Decision at stretch trigger.
